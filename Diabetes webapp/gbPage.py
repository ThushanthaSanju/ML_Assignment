...
def load_model():
    ...
    with open('gb_classifier.pkl', 'rb') as file:
...
def show_gb_page():
    ...
    race = st.selectbox("Select your race ",race, key="race_Opt")
    age = st.slider("age",0,100,35, key="age_Opt")
    gender = st.selectbox("Select your gender ",gender, key="gender_Opt")
    metformin = st.selectbox("Select the relevant check box for metformin",metformin, key="metformin_Opt")
    insulin = st.selectbox("Select the relevant check box insulin",insulin, key="insulin_Opt")
    ...
    if ok:
     ...
     ###############################################################################################
        ...
        diabetes = gb.predict(X)
        st.subheader(f"The Probability of having Diabetes is {diabetes[0]}")